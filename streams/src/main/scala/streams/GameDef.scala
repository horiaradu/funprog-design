...
trait GameDef {
  ...
  case object Left extends Move {
    def change(block: Block) = block.left
  }

  case object Right extends Move {
    def change(block: Block) = block.right
  }

  case object Up extends Move {
    def change(block: Block) = block.up
  }

  case object Down extends Move {
    def change(block: Block) = block.down
  }
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    /**
      ...
      */
    def neighbors: List[(Block, Move)] = moves.map(move => (move.change(this), move))
